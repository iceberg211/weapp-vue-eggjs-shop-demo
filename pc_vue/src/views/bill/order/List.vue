    ...
    >
      <el-table-column prop="createdTime" label="单号 / 下单时间" sortable="custom">
        <template slot-scope="scope">
          <router-link
            class="text-bold"
            :to="{ name: 'orderView', params: { orderUuid: scope.row.uuid } }"
          >
      ...
      <el-table-column prop="paymentAmount" label="商品金额" sortable="custom">
        <template slot-scope="scope">
          <span v-if="scope.row.paymentAmount" class="text-red text-bold"
      ...
      <el-table-column label="状态" align="center" width="100">
        <template slot-scope="scope">
          <span class="badge" :class="scope.row.status | formatOrderStatusToClass">{{
            scope.row.status | formatOrderStatusToCN
          }}</span>
        </template>
      </el-table-column>
      <el-table-column
